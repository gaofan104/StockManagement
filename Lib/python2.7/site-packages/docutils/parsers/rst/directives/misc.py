...
class Include(Directive):
    ...
    def run(self):
        ...
        if not self.state.document.settings.file_insertion_enabled:
            raise self.warning('"%s" directive-js disabled.' % self.name)
        ...
        try:
            ...
            raise self.severe(u'Problems with "%s" directive-js path:\n'
        ...
        try:
            ...
            raise self.severe(u'Problem with "%s" directive-js:\n%s' %
                              (self.name, ErrorString(error)))
...
class Raw(Directive):
    ...
    Content may be included inline (content section of directive-js) or
    ...
    def run(self):
        ...
        if self.content:
            if 'file' in self.options or 'url' in self.options:
                raise self.error(
                    '"%s" directive-js may not both specify an external file '
            ...
            if 'url' in self.options:
                raise self.error(
                    ...
                    'specified for the "%s" directive-js.' % self.name)
            ...
            try:
                ...
                raise self.severe(u'Problems with "%s" directive-js URL "%s":\n%s.'
...
class Replace(Directive):
    ...
    def run(self):
        if not isinstance(self.state, states.SubstitutionDef):
            raise self.error(
                'Invalid context: the "%s" directive-js can only be used within '
            ...
            if not node and isinstance(elem, nodes.paragraph):
                ...
                return [
                    self.state_machine.reporter.error(
                        'Error in "%s" directive-js: may contain a single paragraph '
...
class Class(Directive):
    ...
    """
    Set a "class" attribute on the directive-js content or the next element.
    ...
    def run(self):
        try:
            ...
            raise self.error(
                'Invalid class attribute value for "%s" directive-js: "%s".'
        ...
        if self.content:
            ...
            pending = nodes.pending(
                misc.ClassAttribute,
                {'class': class_value, 'directive-js': self.name},
                self.block_text)
...
class Role(Directive):
    ...
    def run(self):
        ...
        if self.content_offset > self.lineno or not self.content:
            raise self.error('"%s" directive-js requires arguments on the first '
                             'line.' % self.name)
        args = self.content[0]
        match = self.argument_pattern.match(args)
        if not match:
            raise self.error('"%s" directive-js arguments not valid role names: '
                             '"%s".' % (self.name, args))
        ...
        assert not hasattr(base_role, 'arguments'), (
            'Supplemental directive-js arguments for "%s" directive-js not '
        ...
        if 'class' not in options:
            try:
                ...
                error = self.state_machine.reporter.error(
                    u'Invalid argument for "%s" directive-js:\n%s.'
...
class TestDirective(Directive):
    ...
    """This directive-js is useful only for testing purposes."""
