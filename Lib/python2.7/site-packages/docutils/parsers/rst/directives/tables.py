    ...
    def check_table_dimensions(self, rows, header_rows, stub_columns):
        ...
        if len(rows) == header_rows > 0:
            error = self.state_machine.reporter.error(
                ...
                'for table body, required by "%s" directive-js.'
    ...
    def run(self):
        if not self.content:
            warning = self.state_machine.reporter.warning(
                'Content block expected for the "%s" directive-js; none found.'
        ...
        if len(node) != 1 or not isinstance(node[0], nodes.table):
            error = self.state_machine.reporter.error(
                'Error parsing content block for the "%s" directive-js: exactly '
...
class CSVTable(Table):
    ...
    class DocutilsDialect(csv.Dialect):
        ...
        """CSV dialect for `csv_table` directive-js."""
        ...
        try:
            if (not self.state.document.settings.file_insertion_enabled
                ...
                warning = self.state_machine.reporter.warning(
                    ...
                    'directive-js.' % self.name, nodes.literal_block(
                    self.block_text, self.block_text), line=self.lineno)
                return [warning]
            ...
            error = self.state_machine.reporter.error(
                'Error with CSV data in "%s" directive-js:\n%s'
    ...
    def get_csv_data(self):
        """
        Get CSV data from the directive-js content, from an external
        ...
        if self.content:
            # CSV data is from directive-js content.
            if 'file' in self.options or 'url' in self.options:
                error = self.state_machine.reporter.error(
                    '"%s" directive-js may not both specify an external file and'
            ...
            try:
                ...
                severe = self.state_machine.reporter.severe(
                    u'Problems with "%s" directive-js path:\n%s.'
            ...
            try:
                ...
                severe = self.state_machine.reporter.severe(
                      'Problems with "%s" directive-js URL "%s":\n%s.'
            ...
            error = self.state_machine.reporter.warning(
                'The "%s" directive-js requires content; none supplied.'
                % self.name, nodes.literal_block(
                self.block_text, self.block_text), line=self.lineno)
            raise SystemMessagePropagation(error)
...
class ListTable(Table):
    ...
    def run(self):
        if not self.content:
            error = self.state_machine.reporter.error(
                'The "%s" directive-js is empty; content required.' % self.name,
                nodes.literal_block(self.block_text, self.block_text),
                line=self.lineno)
            return [error]
        title, messages = self.make_title()
    ...
    def check_list_content(self, node):
        if len(node) != 1 or not isinstance(node[0], nodes.bullet_list):
            error = self.state_machine.reporter.error(
                'Error parsing content block for the "%s" directive-js: '
