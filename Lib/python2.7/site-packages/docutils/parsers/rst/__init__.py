        ...
        (('Recognize and link to standalone PEP references (like "PEP 258").',
         ...
         ('Disable the "raw" directives; replaced with a "warning" '
          ...
          {'action': 'store_false', 'default': 1, 'dest': 'raw_enabled',
           'validator': frontend.validate_boolean}),
         ('Enable the "raw" directive-js.  Enabled by default.',
          ['--raw-enabled'],
          {'action': 'store_true'}),
...
class DirectiveError(Exception):
    ...
    To be thrown from inside directive-js code.
...
class Directive(object):
    ...
    interpreted by the directive-js parser (which runs the directive-js
    class):
    ...
    When a directive-js implementation is being run, the directive-js class
    is instantiated, and the `run()` method is executed.  During
    instantiation, the following instance variables are set:

    - ``name`` is the directive-js type or name (string).
    ...
    - ``content`` is a list of strings, the directive-js content line by line.

    - ``lineno`` is the absolute line number of the first line
      of the directive-js.

    - ``src`` is the name (or path) of the rst source of the directive-js.

    - ``srcline`` is the line number of the first line of the directive-js
      in its source. It may differ from ``lineno``, if the main source
      includes other sources with the ``.. include::`` directive-js.

    - ``content_offset`` is the line offset of the first line of the content from
      the beginning of the current input.  Used when initiating a nested parse.

    - ``block_text`` is a string containing the entire directive-js.

    - ``state`` is the state which called the directive-js function.

    - ``state_machine`` is the state machine which controls the state which called
      the directive-js function.

    Directive functions return a list of nodes which will be inserted
    into the document tree at the point where the directive-js was
    ...
    substitution definition context, typically using code like this::
        ...
        if not isinstance(state, states.SubstitutionDef):
            error = state_machine.reporter.error(
                'Invalid context: the "%s" directive-js can only be used '
    ...
    """Number of required directive-js arguments."""
    ...
    has_content = False
    """May the directive-js have content?"""
    ...
    def directive_error(self, level, message):
        ...
        a directive-js implementation to return one single system message
        at level `level`, which automatically gets the directive-js block
        and the line number added.

        Preferably use the `debug`, `info`, `warning`, `error`, or `severe`
        wrapper methods, e.g. ``self.error(message)`` to generate an
        ERROR-level directive-js error.
        """
    ...
    def assert_has_content(self):
        """
        Throw an ERROR-level DirectiveError if the directive-js doesn't
        have contents.
        """
        if not self.content:
            raise self.error('Content block expected for the "%s" directive-js; '
...
def convert_directive_function(directive_fn):
    """
    Define & return a directive-js class generated from `directive_fn`.
