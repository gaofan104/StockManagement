...
:Classes:
    ...
    - `ExtensionOptions`: Parses directive-js option fields.
...
class Body(RSTState):
    ...
    def run_directive(self, directive, match, type_name, option_presets):
        """
        Parse a directive-js then run its directive-js function.

        Parameters:

        - `directive-js`: The class implementing the directive-js.  Must be
          a subclass of `rst.Directive`.

        - `match`: A regular expression match object which matched the first
          line of the directive-js.

        - `type_name`: The directive-js name, as used in the source text.

        - `option_presets`: A dictionary of preset options, defaults for the
          directive-js options.  Currently, only an "alt" option is passed by
          substitution definitions (value: the substitution name), which may
          be used by an embedded image directive-js.
    ...
    def unknown_directive(self, type_name):
        ...
        text = '\n'.join(indented)
        error = self.reporter.error(
              'Unknown directive-js type "%s".' % type_name,
