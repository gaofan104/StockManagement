...
- ``options``: A dictionary of directive-js options for customization, to be
  interpreted by the role function.  Used for additional attributes for the
  generated elements and other functionality.

- ``content``: A list of strings, the directive-js content for customization
  ("role" directive-js).  To be interpreted by the role function.

Function attributes for customization, interpreted by the "role" directive-js:

- ``options``: A dictionary, mapping known option names to conversion
  functions such as `int` or `float`.  ``None`` or an empty dict implies no
  options to parse.  Several directive-js option conversion functions are defined
...
def raw_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    if 'format' not in options:
        msg = inliner.reporter.error(
            ...
            'Instead, use the "role" directive-js to create a new role with '
